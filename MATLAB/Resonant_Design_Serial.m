%% Resonant Simulation with Serial Transmission
% Matthew Roberts
% Fall 2020
% Master's Thesis: Educational Demos for Resonant Converter Phenomena
% Displays Hypothetical Data with Simulation from LTSPICE and Actual Board
% data. Use while operating with converter. Refer to paper for manual.
...
HB_switch = 0;                  % Set to 1 for Half-Bridge
                                ...
                                % Reflect both simulation and board for 
                                % values above
                                                               
R_L_vals = [10,20,30,40,50];    % Vary Load values
Sim_R_L_val = 40;               % Define Load R from Simulation
Board_R_L_val = 40;             % Define Load R from Board
Gain_Axis_Lim = 6;              % Define Plot Y limit of Gain Curves

port = "COM16";                 % Set Serial Port
%serialportlist                 
                                % Uncomment to find what ports are open. 
                                % COM16 ~should~ be the port that
                                % the XDS110 Probe talks with the PC on.
                                
pause_time = 15;
                                % If you are getting errors about matrix sizes 
                                % not matching, increase this value.
                                % If you are waiting for a while before the message
                                % below, decrease this value.
...
format shortG
warning('off')
Plot_Setup = [C_switch, ~HB_switch, Sim_R_L_val, Board_R_L_val, Gain_Axis_Lim];
...
sendChar = 'P';
data = uint16.empty(3, 0);

TI_serial = serialport(port, 9600, "Timeout", 10);
disp("Serial Port Opened.")
write(TI_serial, sendChar, "uint16");
disp("Sending Start Character... Transmission may have started already.")
TI_serial.UserData = data;
configureCallback(TI_serial, "byte", 6, @Serial_Callback)

pause(pause_time)               
disp("Transmission Done!")

serial_freq = double(TI_serial.UserData(4:end-1, 1)) * 100;
serial_vin = double(TI_serial.UserData(4:end-1, 2)) + ... 
    (double((TI_serial.UserData(4:end-1, 3))) ./ double(10000));
serial_vout = double(TI_serial.UserData(4:end-1, 4)) + ... 
    (double((TI_serial.UserData(4:end-1, 5))) ./ double(10000));
serial_gain = serial_vout ./ serial_vin;
...
b.Callback = @(es,ed) plot_Gain(R_L_vals, C_r_vals, L_r_vals, L_n_vals, ...
    es.Value, 1, fig, f_r, serial_freq, serial_gain, sim_data, ...
    Plot_Setup); 

% Function to plot Curve
L_n = plot_Gain(R_L_vals, C_r_vals, L_r_vals, L_n_vals, L_n, 0, fig, ...
    f_r, serial_freq, serial_gain, sim_data, Plot_Setup);
...
b.Callback = @(es,ed) plot_Gain_2(R_L_vals, C_r_vals, L_r_vals, ... 
    L_n_vals, es.Value, 1, fig2, f_r, serial_freq, serial_gain, ... 
    sim_data, Plot_Setup); 
...
function L_n = plot_Gain(R_L_vals, C_r_vals, L_r_vals, L_n_vals, L_n, ...
    overwrite, fig, f_r, serial_freq, serial_gain, sim_data, Plot_Setup)
    ...
    for c = 1:length(R_L_mat(:,1))
        ...
        % Add Simulation and Board Measurements to Plot
        serial_norm = serial_freq ./ (f_r(c));
        if((round((c / length(R_eq_mat(:, 1))) - 0.000001) ... 
            ...
            plots(end+1) = plot(serial_norm, serial_gain, 'm--');
            Board_label = char(arrayfun(@char, vpa(Plot_Setup(4)), ...
                'uniform', 0));
            labels2(end+1) = strcat("Brd, R_{L} = ", Board_label(1:2));
        end
...
function L_n = plot_Gain_2(R_L_vals, C_r_vals, L_r_vals, L_n_vals, L_n, ...
    overwrite, fig2, f_r, serial_freq, serial_gain, sim_data, Plot_Setup)
    ...
    for c = 1:length(R_L_mat(:,1))
        ...
        if((round(((c / length(R_eq_mat(:,1)))) - 0.000001) ... 
                == Plot_Setup(2)) && (mod(c, 2) == Plot_Setup(1)))
            plots(end+1) = plot(sim_data(:,1), sim_data(:,2), 'g--');
            Sim_label = char(arrayfun(@char, vpa(Plot_Setup(3)), ...
                'uniform', 0));
            labels2(end+1) = strcat("Sim, R_{L} = ", Sim_label(1:2));
            plots(end+1) = plot(serial_freq, serial_gain, 'm--');
            Board_label = char(arrayfun(@char, vpa(Plot_Setup(4)), ...
                'uniform', 0));
            labels2(end+1) = strcat("Brd, R_{L} = ", Board_label(1:2));
        end
...
function Serial_Callback(src, ~)
    ...
    % Expects frequency first, then Vin, then Vout
    data_temp = read(src, 21, "char");
    data_strings = strsplit(strcat(data_temp), ",");
    
    % Configure received string data to numeric data
    src.UserData(end+1, 1) = transpose(str2double(data_strings(:,1)));
    src.UserData(end, 2) = transpose(str2double(data_strings(:,2)));
    src.UserData(end, 3) = transpose(str2double(data_strings(:,3)));
    src.UserData(end, 4) = transpose(str2double(data_strings(:,4)));
    src.UserData(end, 5) = transpose(str2double(data_strings(:,5)));
